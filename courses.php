<!doctype html>
<html class="no-js" lang="en" dir="ltr">
  <head>
    <!-- HEAD -->
    <?php include("includes/head.html") ?>
    <title>The Solent SCITT | Course Information</title>
  </head>
  <body>
    <!-- TOPNAV -->
    <?php include("includes/topnav.html") ?>

    <!-- HEADER -->
    <div id="page-header">
      <header class="row">
        <div class="columns">
          <h1 class="capitalize">Course Information</h1>
        </div>
      </header>
    </div>

    <section class="row">
      <div class="medium-6 column text-center">
        <h3>UCAS Provider Code: <strong>S56</strong></h3>
      </div>
      <div class="medium-6 column text-center">
        <div class="toggleSwitch">
          <p>Hide Unavailable Courses:</p>
          <div class="switch">
            <input class="switch-input" id="hideUnavailable" type="checkbox" name="hideUnavailable">
            <label class="switch-paddle" for="hideUnavailable">
              <span class="show-for-sr">Hide Unavailable Courses</span>
            </label>
          </div>
        </div>
      </div>


      <div class="column">

        <!-- courseBiology -->
        <div class="medium-6 large-3 column text-center ">
          <div class="course-container">
            <p class="course-title">Biology</p>
            <div class="course-image">
              <svg viewBox="0 0 504.84 402.59">
                <path d="M369,402.59l11-18s-20.91-8.49-28-11c-48-17-84-52-84-127,0-18.5,1-55.52,0-74-6-116-68-140-111-160C121.31-4,0,.59,0,.59s29.35,90.61,40,163c5,34,19,80,51,115,33.62,36.77,139,72,139,72s-65-61-74-115c-11.16-67-58-144-58-144s61.44,34.55,79,96c16,56,42.81,109.32,79,153C285,375.59,369,402.59,369,402.59Z"/>
                <path d="M298,265.59s-16-115,69-152c31-13.51,114-73,114-73s53.65,108.51,0,176c-31,39-155,57-155,57s1.71-32.26,57.5-66.5c44-27,37.5-60.5,37.5-60.5s-19.5,37.5-54,50C314.11,215.75,298,265.59,298,265.59Z"/>
              </svg>
            </div>
            <p class="course-code"><small>SCITT Code</small><br>2CLV</p>
            <p class="course-select"><a class="button accentbutton" data-open="courseBiology">Course Details</a></p>
          </div>
        </div>
        <!-- courseChemistry -->
        <div class="medium-6 large-3 column text-center ">
          <div class="course-container">
            <p class="course-title">Chemistry</p>
            <div class="course-image">
              <svg viewBox="0 0 495.34 494.17">
                <path d="M349.34,0l-35,34,16,17S79.08,301.72,31.34,350c-88,89,28,200,118,112,82.16-80.33,296-296,296-296l16,16,34-33Zm-226,423c-49,52-100,0-51-49,38-38,142-140,142-140h91S171,372.43,123.34,423Z"/>
              </svg>
            </div>
            <p class="course-code"><small>SCITT Code</small><br>2CLW</p>
            <p class="course-select"><a class="button accentbutton" data-open="courseChemistry">Course Details</a></p>
          </div>
        </div>
        <!-- courseComputerScience -->
        <div class="medium-6 large-3 column text-center inactive">
          <div class="course-container">
            <p class="course-title">Computer Science</p>
            <div class="course-image">
              <svg viewBox="0 0 366 334">
                <path d="M325,0H41A41.12,41.12,0,0,0,0,41V244a41.12,41.12,0,0,0,41,41H159v17s-22.38-.54-35,4c-25,9-40.39,28-21,28H262c19.36,0,5-22-26-30-11-2.85-29-2-29-2V285H325a41.12,41.12,0,0,0,41-41V41A41.12,41.12,0,0,0,325,0ZM279,272a10,10,0,1,1,10-10A10,10,0,0,1,279,272Zm41-38a5,5,0,0,1-5,5H52a5,5,0,0,1-5-5V49a5,5,0,0,1,5-5H315a5,5,0,0,1,5,5Z"/>
              </svg>
            </div>
            <p class="course-code"><small>SCITT Code</small><br>Unavailable</p>
            <p class="course-select"><a class="button accentbutton" data-open="courseComputerScience">Course Details</a></p>
          </div>
        </div>
        <!-- courseDance -->
        <div class="medium-6 large-3 column text-center">
          <div class="course-container">
            <p class="course-title">Dance</p>
            <div class="course-image">
              <svg viewBox="0 0 329.1 334">
                <path d="M214.8,112.5c0,0-14.4-25.4-19-34c-7-13-26-4-18,13c4.4,9.3,16.4,29.8,23,42c7,13,19,4,19,4l16-11l-2,52
                  c0,0-27.8,34.5-37,46c-8,10-4,20-4,20s11,45.1,15,61c5,20,29,12,28,1c-0.3-3.1-7-57-7-57l23,5c0,0,11,48.6,14,65c4,22,28,14,27,0
                  c-1.2-17.1-10-68-10-68s10-1,17-4c15.2-6.5,15-9,15-9l-27-60l6-57l12,3c0,0-4.2,24-5,33c-1,11,14,17,17,7s9.4-36.5,11-49
                  c1-8,0-13-11-16c-15.3-4.2-36.5-9-54-9C238.8,90.5,214.8,112.5,214.8,112.5z"/>
                <circle cx="260.8" cy="54.5" r="27"/>
                <path d="M57.8,213.5l26,23c0,0-45.4,68.3-60,92c-8,13-32,1-21-18C17.2,285.7,57.8,213.5,57.8,213.5z"/>
                <path d="M119.8,75.5c4.2-1.7,54.1-54.9,69-71c12-13,29,5,19,17c-19.1,22.9-72.5,80.7-75,86c-7,15-18,82-18,82
                  s17,23.2,23,33c1.8,3,8.2,14.6,9,18c5.1,20.8,11.7,59,14,79c2,17-27,19-30,4c-3.8-19-16-75-16-75c-2-5-43.1-33.5-52-49
                  c-2.6-4.6-4-15.7-4-21c0.1-14.8,13-64,13-64l-29,7c0,0-7.6,27.8-14,47c-4,12-28,10-23-8c4.7-16.8,11.8-35,16-54c2-9,12-9,28-13
                  C73.6,87.6,114.8,77.5,119.8,75.5z"/>
                <circle cx="95.8" cy="44.5" r="29"/>
              </svg>
            </div>
            <p class="course-code"><small>SCITT Code</small><br>39Z7</p>
            <p class="course-select"><a class="button accentbutton" data-open="courseDance">Course Details</a></p>
          </div>
        </div>
        <!-- courseDesignTechnology -->
        <div class="medium-6 large-3 column text-center ">
          <div class="course-container">
            <p class="course-title">Design Technology</p>
            <div class="course-image">
              <svg viewBox="0 0 76.4 77.4">
                <path d="M63.7,4.7c-3-3-6,0-6,0l-49,50l14,14l50-51c0,0,2-2,0-4S65.7,6.7,63.7,4.7z M64.7,22.7l-11-11l3-3l11,11
                  L64.7,22.7z"/>
                <g>
                  <polygon points="74.7,61.7 69.7,66.7 68.7,65.7 73.7,60.7 71.2,58.2 67.7,61.7 66.7,60.7 70.2,57.2 67.2,54.2
                    61.7,59.7 60.7,58.7 66.2,53.2 63.7,50.7 59.7,54.7 58.7,53.7 62.7,49.7 59.7,46.7 54.7,51.7 53.7,50.7 58.7,45.7 56.2,43.2
                    52.7,46.7 51.7,45.7 55.2,42.2 53.2,40.2 39.7,53.7 62.7,76.7 75.7,62.7 	"/>
                  <polygon points="36.2,23.2 34.7,21.7 29.7,26.7 28.7,25.7 33.7,20.7 30.7,17.7 26.7,21.7 25.7,20.7 29.7,16.7
                    27.2,14.2 21.7,19.7 20.7,18.7 26.2,13.2 24.2,11.2 20.7,14.7 19.7,13.7 23.2,10.2 20.2,7.2 14.7,12.7 13.7,11.7 19.2,6.2
                    16.7,3.7 13.7,6.7 12.7,5.7 15.7,2.7 13.7,0.7 0.7,14.7 22.7,36.7 	"/>
                </g>
                <path d="M7.7,56.7l13,13c0,0-17,6-18,5S7.7,56.7,7.7,56.7z"/>
              </svg>
            </div>
            <p class="course-code"><small>SCITT Code</small><br>32QS</p>
            <p class="course-select"><a class="button accentbutton" data-open="courseDesignTechnology">Course Details</a></p>
          </div>
        </div>
        <!-- courseEnglish -->
        <div class="medium-6 large-3 column text-center">
          <div class="course-container">
            <p class="course-title">English</p>
            <div class="course-image">
              <svg viewBox="0 0 197.5 189">
              	<path d="M185.8,30c-10,39-28,103-38,135c-0.9,3-5.8,9-9,9c-23,0-93,0-114,0c-10,0-14-15-5-15c19,0,71,0,107,0
              		c3.7,0,10-6.4,11-10c9-34,27-98,36-134c1.1-4.2,0.1-15-9-15c-29,0-82,0-110,0c-4,0-12,5.2-13,9c-8,31-29,106.8-40,142
              		c-5,16,0,38,24,38c18,0,89-1,123,0c4.1,0.1,11.1-7,12-11c7-31,26.9-101,36-135C200.8,28,186.9,25.5,185.8,30z M120.6,50H76.3l1.8-7
              		h43.7L120.6,50z M67.8,29h73l-2,7h-74L67.8,29z"/>
              </svg>
            </div>

            <p class="course-code"><small>SCITT Code</small><br>32QT</p>
            <p class="course-select"><a class="button accentbutton" data-open="courseEnglish">Course Details</a></p>
          </div>
        </div>
        <!-- courseGeography -->
        <div class="medium-6 large-3 column text-center">
          <div class="course-container">
            <p class="course-title">Geography</p>
            <div class="course-image">
              <svg viewBox="0 0 126 147">
                <path d="M111.4,96.5c0.2-0.2,0.4-0.4,0.5-0.6c-1.1-1.4-2.4-3.4-2.9-5.9c-0.4-1.8-5.5,1-7,0c-1.5-1-1.8-5.4-1-7
                  c1.1-2.3,6.6-6,9-5c1,0.4,0.9,3.8,2,4c1.4,0.2,2.6-4,4-4c1.5,0,2.6,3.6,3.9,5.8C123.8,76,126,67.3,126,58c0-1.4-0.1-2.7-0.2-4.1
                  c-1.8,5.7-4.6,12.4-7.8,13.1c-2.6,0.5-5.4-7.4-8-7c-3.2,0.5-3.7,10.9-7,11c-3.4,0.1-4.6-10.9-8-11c-1.6,0-2.4,4.8-4,5
                  c-2.8,0.3-6.9-8.9-9-7c-2.4,2.2,6.6,7.8,7,11c0.6,4.4-4.3,13.5-8,16c-2.1,1.4-8,2.6-10,1c-3.2-2.5,1-13.3-2-16c-1.5-1.3-6.5,1.3-8,0
                  c-2.5-2.1-3.7-10.2-2-13c1.2-2,9.1-0.6,9-3c0-1.8-3.8-0.6-5-2c-1.8-2.1-3.3-9.5-1-11c1.7-1.1,5.3,5,7,4c1.5-0.9-2.6-5.3-2-7
                  c0.8-2.4,3.7-7,6-8c3.2-1.4,10.7,1.3,14,0c3.5-1.3,7.5-8.7,11-10c2.7-1,8.9-0.6,13.8,0C101.2,7.7,85.5,0,68,0
                  C53.2,0,39.7,5.6,29.4,14.7c2.7,0.7,6.8,1.7,8.6,1.3c1.6-0.4,3.5-3.6,5-4c3.7-1,13.8,0.3,15,4c2,5.9-5.9,23.2-12,22
                  c-3.9-0.8-1.7-17.2-5-15c-3,2,2.3,16.3,0,21c-2.9,5.9-21,16-21,16s4.7,6,7,6c14,0,21.2,5.1,21,11c-0.2,5-9.8,11.5-12,16
                  c-1,2,1.2,8.4-1,9c-2.5,0.6-5-9-5-9V80c0,0-4-1-4-11c0,0-9.8-3.1-16-10.6C10.2,90.2,36.1,116,68,116c15.2,0,29.1-5.9,39.4-15.5
                  c0.8,0.9,2,2.2,3.3,3.7C99.5,114.6,84.5,121,68,121C33.2,121,5,92.6,5,57.5c0-16.4,6.2-31.4,16.3-42.6C23.4,17.2,25,19,25,19l4-4
                  c0,0-8-9-11-12c-2-2-6,2-4,4c1,1,2.4,2.5,3.9,4.1C6.8,23.2,0,39.3,0,57c0,35.2,26.7,64.1,61,67.6V139c0,0-10,0-13,0c-4,0-5,8,0,8
                  c9,0,21,0,32,0c4,0,3-8,0-8c-4,0-12,0-12,0v-14c17.6,0,33.6-6.7,45.6-17.6c1.5,1.6,2.9,3.1,3.8,4.1c2,2,6-2,4-4
                  C118.7,104.7,113.4,98.8,111.4,96.5z"/>
              </svg>
            </div>
            <p class="course-code"><small>SCITT Code</small><br>33Z2</p>
            <p class="course-select"><a class="button accentbutton" data-open="courseGeography">Course Details</a></p>
          </div>
        </div>
        <!-- courseHistory -->
        <div class="medium-6 large-3 column text-center">
          <div class="course-container">
            <p class="course-title">History</p>
            <div class="course-image">
              <svg viewBox="0 0 300.65 284">
                <path d="M158.94,3A139,139,0,0,0,22,116.49c-.17.94-9.75-.07-17.37.12a1.67,1.67,0,0,0-1.3,2.66L30,163.42a5,5,0,0,0,7.41.75c9.52-8.74,23.11-35.92,28.58-44.53a1.69,1.69,0,0,0-1.45-2.6c-5.47.06-16.24.14-16.17-.17A112.53,112.53,0,0,1,159.65,29c61.65.8,111.52,51.8,111,113.46A112.51,112.51,0,0,1,65.55,205.4c-.55-.79-1.07-1.6-1.6-2.4H33.73c.36.75.72,1.49,1.1,2.23A139,139,0,0,0,297.65,141C297.11,65,234.88,3.15,158.94,3Z"/>
                <polygon points="150.65 54 150.65 147 216.65 185 223.65 174 163.65 139 163.65 54 150.65 54"/>
              </svg>
            </div>
            <p class="course-code"><small>SCITT Code</small><br>33Z3</p>
            <p class="course-select"><a class="button accentbutton" data-open="courseHistory">Course Details</a></p>
          </div>
        </div>
        <!-- courseMaths -->
        <div class="medium-6 large-3 column text-center">
          <div class="course-container">
            <p class="course-title">Mathematics</p>
            <div class="course-image">
              <svg viewBox="0 0 300 301">
                <path d="M135,0C102,0,41,0,5,0C3.2,0,0,3.2,0,5c1,38,0,101,0,133c0,1.4,2.6,4,4,4c34,0,93,0,132,0c1.8,0,5-3.2,5-5
                  c0-33,0-95,0-132C141,3,137,0,135,0z M103,79H76v28H66V79H38V69h28V41h10v28h27V79z"/>
                <path d="M294,0c-33,0-94,0-130,0c-1.8,0-5,3.2-5,5c1,38,0,101,0,133c0,1.4,2.6,4,4,4c34,0,93,0,132,0c1.8,0,5-3.2,5-5
                  c0-33,0-95,0-132C300,3,296,0,294,0z M262.5,78.5h-66v-9h66V78.5z"/>
                <path d="M135,159c-33,0-94,0-130,0c-1.8,0-5,3.2-5,5c1,38,0,101,0,133c0,1.4,2.6,4,4,4c34,0,93,0,132,0c1.8,0,5-3.2,5-5
                  c0-33,0-95,0-132C141,162,137,159,135,159z M97,252l-7,7l-20-20l-19,20l-7-7l19-20l-20-19l7-7l20,19l19-19l8,8l-19,19L97,252z"/>
                <path d="M294,159c-33,0-94,0-130,0c-1.8,0-5,3.2-5,5c1,38,0,101,0,133c0,1.4,2.6,4,4,4c34,0,93,0,132,0c1.8,0,5-3.2,5-5
                  c0-33,0-95,0-132C300,162,296,159,294,159z M232,200c3.9,0,7,3.1,7,7c0,3.9-3.1,7-7,7s-7-3.1-7-7C225,203.1,228.1,200,232,200z
                   M232,255c-3.9,0-7-3.1-7-7c0-3.9,3.1-7,7-7s7,3.1,7,7C239,251.9,235.9,255,232,255z M269,231h-76v-7h76V231z"/>
              </svg>
            </div>
            <p class="course-code"><small>SCITT Code</small><br>33Z4</p>
            <p class="course-select"><a class="button accentbutton" data-open="courseMaths">Course Details</a></p>
          </div>
        </div>
        <!-- courseMFL -->
        <div class="medium-6 large-3 column text-center">
          <div class="course-container">
            <p class="course-title">Modern Foreign Languages</p>
            <div class="course-image">
              <svg viewBox="0 0 1366 768">
                <style type="text/css">
                  .st0{fill:#FFFFFF;}
                  .st1{font-family:'MyriadPro-Regular';}
                  .st2{font-size:310px;}
                  .st3{font-size:254.8266px;}
                </style>
                <g>
                  <path d="M826.5,533.5c-133.4,0-242-89.5-242-199.5s108.6-199.5,242-199.5s242,89.5,242,199.5c0,31-8.4,60.8-25,88.4
                    c-9.1,15.2,9.5,30,35.2,50.6c6.2,5,15.2,12.1,21.4,17.8c-0.1,0-0.3,0-0.5,0c-12.5,0-30.8-2-48.5-3.9c-18.5-2-35.9-3.9-48.7-3.9
                    c-10.5,0-16.8,1.3-21.2,4.3C937.9,517.1,883,533.5,826.5,533.5z"/>
                  <path d="M826.5,146c127.1,0,230.5,84.3,230.5,188c0,28.9-7.9,56.7-23.4,82.5c-7.9,13.2-4.1,27.7,11.5,43.1
                    c5.6,5.5,12.7,11.4,20,17.3c-4.2-0.4-8.5-0.9-12.7-1.4c-18.8-2-36.5-4-49.9-4c-13,0-21.3,1.9-27.7,6.3
                    c-20.3,13.9-43.1,24.8-67.8,32.4c-25.7,7.8-52.8,11.8-80.5,11.8C699.4,522,596,437.7,596,334S699.4,146,826.5,146 M826.5,123
                    c-140,0-253.5,94.5-253.5,211s113.5,211,253.5,211c61.3,0,117.5-18.1,161.3-48.2c2.4-1.6,7.6-2.3,14.7-2.3
                    c24.6,0,71.3,7.8,97.1,7.8c8.8,0,15.1-0.9,17.4-3.3c11-11.5-71.6-57.4-63.7-70.7c17.1-28.4,26.7-60.4,26.7-94.3
                    C1080,217.5,966.5,123,826.5,123L826.5,123z"/>
                </g>
                <text transform="matrix(1 0 0 1 732.0332 426.2847)" class="st0 st1 st2">A</text>
                <g>
                  <path d="M517.6,612.5c-46,0-90.8-13.4-126.1-37.6c-4-2.7-9.5-3.9-18.6-3.9c-10.6,0-25,1.6-40.2,3.2c-12.2,1.3-24.8,2.7-34.5,3.1
                    c4.4-3.8,9.5-7.8,13.3-10.9c22-17.5,37.9-30.2,29.4-44.2c-13.5-22.4-20.3-46.5-20.3-71.6c0-89.3,88.3-161.9,196.9-161.9
                    s196.9,72.6,196.9,161.9S626.2,612.5,517.6,612.5z"/>
                  <path d="M517.6,300.1c102.2,0,185.4,67.5,185.4,150.4c0,83-83.2,150.4-185.4,150.4c-22.4,0-44.2-3.2-65-9.5
                    c-19.9-6.1-38.3-14.8-54.6-26.1c-7.6-5.2-17-5.9-25.1-5.9c-10.5,0-24,1.4-38.5,2.9c2.2-2,4.3-3.9,6.3-5.8
                    c17.8-17.6,15.3-31.7,10.1-40.4c-12.4-20.6-18.6-42.7-18.6-65.7C332.2,367.6,415.4,300.1,517.6,300.1 M517.6,277.1
                    c-115.1,0-208.4,77.7-208.4,173.4c0,27.9,7.9,54.2,21.9,77.5c6.5,10.9-61.4,48.7-52.3,58.1c1.9,2,7.1,2.7,14.3,2.7
                    c21.2,0,59.6-6.4,79.8-6.4c5.8,0,10.1,0.5,12.1,1.9c36,24.8,82.2,39.6,132.6,39.6C632.7,624,726,546.3,726,450.6
                    S632.7,277.1,517.6,277.1L517.6,277.1z"/>
                </g>
                <text transform="matrix(1 0 0 1 452.2813 531.4141)" class="st0 st1 st3">á</text>
              </svg>

            </div>
            <p class="course-code"><small>SCITT Code</small><br>39Z9</p>
            <p class="course-select"><a class="button accentbutton" data-open="courseMFL">Course Details</a></p>
          </div>
        </div>
        <!-- coursePhysics -->
        <div class="medium-6 large-3 column text-center ">
          <div class="course-container">
            <p class="course-title">Physics</p>
            <div class="course-image">
              <svg viewBox="0 0 310 289">
              <g>
              	<path d="M304.9,157.2c0.6-2.5,0.9-5,0.9-7.5c-0.1-21.7-22.7-40.8-57.9-53.2c7.6-37.7,2.6-67.9-16.5-79.1
              		c-19.1-11.2-48.1-0.9-77.4,24.4c-8.5-7.6-17-13.9-25.3-18.6c0-0.4,0-0.8,0-1.1c0-12.2-9.8-22-22-22c-9.3,0-17.2,5.8-20.5,13.9
              		c-2.5,0.7-4.8,1.6-7,2.8C59.6,27.4,53.7,57.9,60.4,96.4C23.6,109.1-0.1,129,0,151.3c0.1,22,23.4,41.4,59.4,53.7
              		c-2,10.9-3,21.1-2.8,30.3c-0.3,0.2-0.7,0.4-1,0.6c-10.4,6.2-13.8,19.8-7.6,30.2c4.8,8,13.8,11.8,22.4,10.4c1.8,1.8,3.8,3.3,6,4.6
              		c18.4,10.8,45.9,1.6,74-21.5c28,25.2,56.1,36,75.2,25.5c19.7-10.8,25.6-41.5,18.7-80.2c12.3-4.2,23.1-9.1,32.2-14.7
              		c0.3,0.2,0.7,0.4,1,0.6c10.7,5.8,24,1.8,29.8-8.9C311.8,173.6,310.5,163.9,304.9,157.2z M223.2,29.1c15.1,8.9,19.2,32.9,13.5,63.8
              		c-13.8-4-29.1-7-45.5-8.8c-9.7-13.6-20-25.7-30.3-35.8C185.1,28.3,208.1,20.2,223.2,29.1z M202.2,180.1
              		c-6,10.2-12.3,19.7-18.8,28.6c-9.1,0.8-18.5,1.3-28.1,1.3c-11.8,0.1-23.3-0.5-34.2-1.6c-6.9-9.9-13.6-20.6-19.9-32
              		c-5.2-9.6-9.9-19.2-13.9-28.6c4.2-8.9,8.9-17.9,14.2-26.9c5.3-9,10.8-17.5,16.6-25.5c11.7-1.4,23.9-2.1,36.6-2.2
              		c11.5-0.1,22.7,0.5,33.4,1.5c5.4,8.1,10.6,16.7,15.6,25.8c5.4,9.9,10.1,19.7,14.3,29.4C213.4,159.8,208.2,170,202.2,180.1z
              		 M223.6,164.2c4.5,12.4,7.9,24.3,10.2,35.5c-10.9,3.1-23,5.6-35.8,7.4c5.1-7.3,10.1-15.1,14.9-23.2
              		C216.8,177.3,220.3,170.7,223.6,164.2z M151.7,244.8c-7.9-7.7-15.9-16.6-23.6-26.7c8.2,0.6,16.5,0.8,25.1,0.8
              		c7.8,0,15.5-0.3,23-0.9C168.2,228.1,160,237.1,151.7,244.8z M106.2,206.6c-13.4-2-25.9-4.8-37-8.4c2.8-11.6,6.9-24.1,12.2-36.9
              		c3.4,7.4,7.1,14.8,11.1,22.3C96.8,191.7,101.4,199.3,106.2,206.6z M82.9,137c-4.3-11.2-7.7-22.1-10.2-32.4
              		C82.9,101.5,94,99,106,97.1c-3.8,5.6-7.5,11.5-11,17.5C90.6,122.1,86.5,129.6,82.9,137z M179.9,83c-8.9-0.7-18-1-27.4-1
              		c-8.6,0-17,0.4-25.2,1.1c9-11.3,18.2-21.2,27.4-29.5C163.2,62.1,171.7,72,179.9,83z M212.5,118.1c-4.2-7.7-8.6-15.1-13.2-22.1
              		c12.6,1.7,24.4,4.1,35.2,7.1c-2.7,11.2-6.6,23.2-11.7,35.5C219.7,131.8,216.2,124.9,212.5,118.1z M84.9,24.9
              		C86.3,35.7,95.5,44,106.7,44c8.9,0,16.5-5.2,20-12.8c6.9,4.1,14,9.5,21.2,16c-11.2,10.4-22.4,22.9-33,37.2
              		c-16,1.9-31.1,4.9-44.6,8.9C64.2,59.9,68.7,33.8,84.9,24.9z M14.2,152.4c-0.1-17.7,18.7-33.6,48.6-44.5c3.2,13.6,7.8,28,13.8,42.6
              		c-6.8,15.6-11.9,30.8-15.2,45.1C32.5,185.1,14.3,169.7,14.2,152.4z M80.6,271.9c8.5-6.8,10.9-18.9,5.1-28.5
              		c-4.6-7.6-13-11.4-21.2-10.6c0-7.7,0.9-16.3,2.6-25.4c13.8,4.1,29.3,7.3,45.9,9.2c9.6,13.6,19.8,25.6,30,35.8
              		C118.9,272.6,95.8,280.8,80.6,271.9z M219.9,272c-15.1,8.3-37.4-0.1-60.5-20.2c10.7-9.9,21.5-21.8,31.7-35.2
              		c15.9-1.9,30.8-4.9,44.3-8.8C240.3,239.2,235.5,263.5,219.9,272z M268.7,160.9c-4.2,7.8-3.2,17,1.8,23.6
              		c-7.8,4.6-17.1,8.8-27.6,12.3c-3-14-7.7-28.8-13.8-44c7.3-16,12.8-31.7,16.5-46.4c30.6,10.5,50.1,26.4,50.2,44.3
              		C285.6,146.9,274,151.1,268.7,160.9z"/>
              	<circle cx="153.7" cy="154" r="42"/>
              </g>
              </svg>
            </div>
            <p class="course-code"><small>SCITT Code</small><br>2CLX</p>
            <p class="course-select"><a class="button accentbutton" data-open="coursePhysics">Course Details</a></p>
          </div>
        </div>
        <!-- coursePhysics -->
        <div class="medium-6 large-3 column text-center ">
          <div class="course-container">
            <p class="course-title">Primary Teaching</p>
            <div class="course-image">
              <svg viewBox="0 0 188.69 121">
                <path d="M125.5,78.5c-3.16-3.13-16-7-14,11,.1.94,1.61-2.17,3.23-5.93,1.77-4.07,3.94-4.07,4.77-4.07,7.07,0,5.53,6.9,5,9-1,4-4,15-24,18a53.35,53.35,0,0,0-9,2c-5,2-6,12,0,12h27c9,0,16-5,16-11,0-2.75.42-8.28,0-11C133.66,93.08,129.4,82.36,125.5,78.5Z"/>
                <path d="M95.5,104.5c-.71-.71,12-7,16.5-10.5,2.68-2.09,4.41-11.23,7-12,1.83-.54,4.5,1.5,4,4-.74,3.68-4,9-7,12C111.5,102.5,96.5,105.5,95.5,104.5Z"/>
                <circle cx="118.75" cy="60.75" r="11.75"/>
                <circle cx="41" cy="13" r="13"/>
                <path d="M3,76c-.25,7.33,10.55,12.25,25.5,13.5,6.41.53,13.79-.19,19.68-2.78C55.5,83.5,53.5,116.5,62.5,120.5c2,.91,5.91,0,6-3,.33-10.68-8.7-35.48-7.57-36.13A14.8,14.8,0,0,0,63,80c1.18-1-13.92-6.3-17.79-19.26Q45.1,60.38,45,60c-.33-1.29-8.92-4.44-11.5-6.5-10-8-10.5-10-12-14-2.41-6.34,2-8.67,4-9,6-1,16.16,15.06,15.5,12.69-.34-1.2-.67-2.29-1-3.19-7.5-20.5-24.5-13.5-28-1C8.16,52.7,3.5,61.5,3,76Z"/>
                <path d="M66,55c3.5.5,5.5,7.5.5,7.5A61.37,61.37,0,0,1,52,60c-8.26-2.88-23.3-12.62-28-20-3.5-5.5,3-8,6-5,3.5,3.5,8.59,9.18,11.5,11.5A39.1,39.1,0,0,0,55,53C57.69,53.59,63.27,54.61,66,55Z"/>
                <path d="M40,91H7L0,121H5l2-10H40l2,10h5ZM8.5,105.5l2-10h26l2,10Z"/>
                <path d="M178.5,78.5c-3.16-3.13-16-7-14,11,.1.94,1.61-2.17,3.23-5.93,1.77-4.07,3.94-4.07,4.77-4.07,7.07,0,5.53,6.9,5,9-1,4-4,15-24,18a53.35,53.35,0,0,0-9,2c-5,2-6,12,0,12h27c9,0,16-5,16-11,0-2.75.42-8.28,0-11C186.66,93.08,182.4,82.36,178.5,78.5Z"/>
                <path d="M148.5,104.5c-.71-.71,12-7,16.5-10.5,2.68-2.09,4.41-11.23,7-12,1.83-.54,4.5,1.5,4,4-.74,3.68-4,9-7,12C164.5,102.5,149.5,105.5,148.5,104.5Z"/>
                <circle cx="171.75" cy="60.75" r="11.75"/>
              </svg>

            </div>
            <p class="course-code"><small>SCITT Code</small><br>3496</p>
            <p class="course-select"><a class="button accentbutton" data-open="coursePrimaryTeaching">Course Details</a></p>
          </div>
        </div>
        <!-- courseReligiousEducation -->
        <div class="medium-6 large-3 column text-center end">
          <div class="course-container">
            <p class="course-title">Religious Education</p>
            <div class="course-image">
              <svg viewBox="0 0 415.52 479.51">
                <path d="M389,79.68C356.86,35,311.55,12.37,258.53,3.2,250.36,1.79,242.08,1,233.85,0h-23c-2,.32-3.91.7-5.87,1-13.18,1.76-26.54,2.65-39.5,5.4-37.73,8-70.36,25.21-95.1,55.58C50.77,86,40.16,114,33.8,144.12c-2.44,11.56-1.72,22.18,4.26,32.22,4.77,8,4.29,15.77.21,23.69a65.17,65.17,0,0,1-4.84,8.71c-7.95,10.9-16.24,21.54-24.08,32.51A73,73,0,0,0,1,255.87c-2.42,5.93-.31,10.92,5.37,13.87,4.1,2.13,8.64,3.42,13,5.07,4.19,1.59,8.57,2.81,12.52,4.85,7,3.59,8.71,9.13,5.56,16.33-1.13,2.58-2.67,5-3.95,7.51-3.58,7-2.48,11.26,4.09,15.72,1.86,1.26,3.74,2.5,6.3,4.22-2.74,2.1-4.81,3.68-6.88,5.28-6.67,5.16-6.4,13.66.77,18.23a41.44,41.44,0,0,0,8.18,3.61c8.21,3,9.34,4.83,7.63,13.39s-3.75,16.89-5.11,25.42c-2.3,14.43,4.69,23.84,19.17,25.38,8.41.9,17,.48,25.43,1,10.28.57,20.7.6,30.77,2.42,13.56,2.46,21.83,11.12,24.47,25,1.56,8.21-.24,15.83-2.73,23.49-2.8,8.6,2.45,14.51,11.12,12.49Q257.23,455.61,357.76,432c8.55-2,10.66-6.23,7.87-14.71-2.91-8.84-5.54-17.78-8.44-26.63-10-30.43-6.42-59.41,9.26-87.1,4.76-8.4,9.38-16.88,14.41-25.12,19.08-31.26,32.28-64.33,34.35-101.49C417.18,141.47,409.79,108.67,389,79.68ZM266.65,282.2c-3.66-.13-7.34,0-11.41,0C252,295.66,245,302,234.18,301.82c-10.26-.18-17.18-6.85-19.85-19.64-3.91,0-8-.07-12,0-4.79.11-7.24-2-7.2-6.93.07-8.58.09-17.17,0-25.75-.06-4.82,2-7.17,6.94-7.16q32.49.08,65,0c4.8,0,7.24,2,7.2,6.91-.08,8.58-.11,17.17,0,25.75C274.3,280.3,271.7,282.38,266.65,282.2Zm41.42-132.51a139.89,139.89,0,0,1-8.28,15c-11.42,18.15-23.59,35.93-25.61,58.31H195.23c-2.31-24.25-16.47-43.07-28.43-62.95-17.79-29.58-17.59-60.29,1.14-84.33,18.56-23.82,43.83-33.31,73.39-31.52,20.91,1.27,39.23,9.3,54,24.4C316.65,90.3,321.47,121.13,308.07,149.69Z"/>
              </svg>
            </div>
            <p class="course-code"><small>SCITT Code</small><br>33Z5</p>
            <p class="course-select"><a class="button accentbutton" data-open="courseReligiousEducation">Course Details</a></p>
          </div>
        </div>



        <!-- MODALS -->
        <!-- courseBiology -->
        <div class="large reveal" id="courseBiology" data-reveal>
          <h2>Biology</h2>
          <p class="lead">Why Biology?</p>
          <p>Biology is the study of life at a number of levels, from cells to how organisms can function.
          <br>Organ systems working together is complex, yet magnificently allows animals to survive many threats, such as mutations and microbial attacks. The study of such systems is fascinating. In humans, the immune system fighting off infection to the circulatory system providing key nutrients to the cells of the body to the nervous system communicating enabling reflex reactions for survival are all important aspects of biological study.</p>
          <p>In plants, the leaves full of chloroplasts enabling water and carbon dioxide using light energy to form glucose is an awe inspiring process. The clever transport system allowing glucose and water to be moved efficiently through the plant helps our understanding of how plants function effectively.
          <br>Together, the study of life can open an interest and passion within pupils that could spark the hunger to achieve the next Biological milestone.
          <br>At the heart of Biology is the excitement of new information and discoveries as a result of new and inspired minds, wanting to follow in the footsteps of Pasteur, Jenner and Darwin. All ahead of their time, could you inspire a new generation of biologists?</p>

          <p class="lead">How is the Subject Study structured?</p>
          <p>Subject specific sessions are run by Subject Experts for each specialism and take place for half a day each week. In addition there will be a weekly half day session in school for subject specialist development.</p>
          <p>Subject sessions being delivered by a Subject Expert who is a current outstanding practitioner ensures that you will get up to date advice on subject specific teaching and learning, and the most effective ways of making this outstanding within the classroom.</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>2CLV</td>
            </tr>
            <tr>
              <td>School Direct</td>
              <td>3B7V</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- courseChemistry -->
        <div class="large reveal" id="courseChemistry" data-reveal>
          <h2>Chemistry</h2>
          <p class="lead">Why Chemistry?</p>
          <p>Chemistry plays a central role in the Sciences, linking key ideas in Physics, Biology, Medicine, Mathematics and Environmental Sciences. At the heart of Chemistry is the study of the world around us; from the design of new life-saving medicines to the exploration of space, Chemistry touches every aspect of life.</p>
          <p>As a trainee at The Solent SCITT, you will develop the knowledge and skills to inspire the next generation of Chemists. You will work in four of the region's leading Science departments alongside dedicated teachers with a range of backgrounds and experience. In addition, you will receive tailored training focused on the key challenges faced by the teaching of this fundamental subject.</p>

          <p class="lead">How is Subject Study structured?</p>
          <p>Subject specific sessions are run by Subject Experts for each specialism and take place for half a day each week. In addition there will be a weekly half day session in school for subject specialist development.</p>
          <p>Subject sessions being delivered by a Subject Expert who is a current outstanding practitioner ensures that you will get up to date advice on subject specific teaching and learning, and the most effective ways of making this outstanding within the classroom.</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>2CLW</td>
            </tr>
            <tr>
              <td>School Direct</td>
              <td>3B7W</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- courseComputerScience -->
        <div class="large reveal" id="courseComputerScience" data-reveal>
          <h2>Computer Science</h2>
          <p class="lead">Why Computer Science?</p>
          <p>Teaching Computer Science with ICT is like surfing the crest of a wave. It is challenging, exhilarating, has massive highs and its direction is ever changing. Computer Science and ICT education is going through the biggest shift in direction it has ever seen. Studying at The Solent SCITT will ensure that you are at the forefront of development of a curriculum area. Pupils need to understand the functionality of software but also program and design the next groundbreaking aspects of technology. You will receive training, support and expertise that will enable you to become a competent Computer Science teacher.</p>
          <p>You will be immersed in the real life world of education on a daily basis supported by people that are equipped to shape and build the outstanding teachers of the future. The Solent SCITT provides an exciting opportunity to learn the skills of the trade in a hands on practical way that will help you to be part of a new cohort of teachers ready to inspire the next Turing or Gates.</p>

          <p class="lead">How is Subject Study structured?</p>
          <p>Subject specific sessions are run by Subject Experts for each specialism and take place for half a day each week. In addition there will be a weekly half day session in school for subject specialist development.</p>
          <p>Subject sessions being delivered by a Subject Expert who is a current outstanding practitioner ensures that you will get up to date advice on subject specific teaching and learning, and the most effective ways of making this outstanding within the classroom.</p>

          <p class="lead">Do I need subject knowledge enhancement?</p>
          <p>There are a number of fully funded subject enhancement courses which can be completed in the Summer term before you start your training.  Once you have secured your training place you will be eligible to apply - don't delay, apply for a SCITT PGCE place today!</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td></td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- courseDesignTechnology -->
        <div class="large reveal" id="courseDesignTechnology" data-reveal>
          <h2>Design Technology</h2>
          <p class="lead">Why Design Technology?</p>
          <p>Design and Technology Education provides an exciting opportunity to enthuse students through the application of knowledge and skills to create innovative products that exude creativity nothing can beat the sense of reward and well being that you have when students show their work with sheer pride and exuberance! Studying Design and Technology at The Solent SCITT will enable you to develop your skills in the classroom first hand with excellent facilities, training and support. You will train within highly performing departments who are adept at developing pupil performance and competence at highly skilled work within Design and Technology.</p>
          <p>Can you help pupils to achieve excellent results? Can you model expertise and show innovation within Design and Technology? Working within teams with a track history of excellent results, an outstanding curriculum, National award winning extra-curricular provision and enthusiastic students there is no better place to throw yourself into your training and experience teaching the subject first hand.</p>

          <p class="lead">How is Subject Study structured?</p>
          <p>Subject specific sessions are run by Subject Experts for each specialism and take place for half a day each week. In addition there will be a weekly half day session in school for subject specialist development.</p>
          <p>Subject sessions being delivered by a Subject Expert who is a current outstanding practitioner ensures that you will get up to date advice on subject specific teaching and learning, and the most effective ways of making this outstanding within the classroom.</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>32QS</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- courseDance -->
        <div class="large reveal" id="courseDance" data-reveal>
          <h2>Dance</h2>
          <p class="lead">Why Dance?</p>
          <p>Dance provides students with an exciting opportunity to develop a wide variety of skills which not only involve creativity and innovation but also helps to develop life skills which are needed when moving on into further education or into the world of work. The Solent SCITT schools all have a strong performing arts ethos with departments that are outstanding. Park Community School is an outstanding school with an outstanding arts department. We have performing arts specialist status along with Artsmark Gold and have an excellent working relationship with many outside organisations and agencies. You will be led by an outstanding Subject Expert and see outstanding Dance teachers whose knowledge and understanding of the subject is excellent and who are looking forward to using their expertise to help others.</p>
          <p>There is a real sense of reward and achievement when you watch your students not only showing their independence and creativity but also when you see them developing life skills such as teamwork, communication, cooperation and leadership which they will take forward with them in the future. We have always achieved excellent results in both GCSE and Btec Dance and this is invaluable when training teachers of the future.</p>
          <p>As a trainee with us you will gain invaluable experience in learning how to develop student's skills effectively. You will have the support of a well-established, outstanding department who will provide you with all the tools you need to become an outstanding teacher yourself. Working within an outstanding team will be challenging but also incredibly rewarding.</p>

          <p class="lead">How is Subject Study structured?</p>
          <p>Subject specific sessions are run by Subject Experts for each specialism and take place for half a day each week. In addition there will be a weekly half day session in school for subject specialist development.</p>
          <p>Subject sessions being delivered by a Subject Expert who is a current outstanding practitioner ensures that you will get up to date advice on subject specific teaching and learning, and the most effective ways of making this outstanding within the classroom.</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>39Z7</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- courseEnglish -->
        <div class="large reveal" id="courseEnglish" data-reveal>
          <h2>English</h2>
          <p class="lead">Why English?</p>
          <p>Read a book that has changed you? Used writing as a therapy? English is an expressive art form, which can change people. Your passion and enthusiasm for English could show pupils the power of effective speaking, listening and writing for different audience and purpose.</p>
          <p>English is undergoing change in schools, with an evolving National Curriculum and new assessment requirements at GCSE. Studying English at The Solent SCITT, will enable you to receive the training, support and expertise to facilitate your transition into this exciting ocean of opportunity.</p>
          <p>We believe that teachers, immersed in this experience on a daily basis, are the people best equipped to train and mentor the teachers of the future; a reason why The Solent SCITT provides such an exciting, relevant and potentially life-changing situation. Could you be the inspiration for pupils to excel in English and create life changing works?</p>

          <p class="lead">How is the Subject Study structured?</p>
          <p>Subject specific sessions are run by Subject Experts for each specialism and take place for half a day each week. In addition there will be a weekly half day session in school for subject specialist development.</p>
          <p>Subject sessions being delivered by a Subject Expert who is a current outstanding practitioner ensures that you will get up to date advice on subject specific teaching and learning, and the most effective ways of making this outstanding within the classroom.</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>32QT</td>
            </tr>
            <tr>
              <td>School Direct</td>
              <td>3B7X</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- courseGeography -->
        <div class="large reveal" id="courseGeography" data-reveal>
          <h2>Geography</h2>
          <p class="lead">Why Geography?</p>
          <p>Geography is arguably one of the most diverse, interesting and important subjects the national curriculum offers. Geography promotes an understanding of the nature of the Earth’s surface and more particularly the character of places, the complex nature of people’s relationships and interactions with their environment, and the importance in human affairs of the location and spatial organisation of human activities. The study of Geography is so vast that there is literally something for everyone. Your enthusiasm for Geography will inspire students for years to come.</p>
          <p>Learn how to teach Geography with passionate and experienced teachers through SCITT and pass on your enthusiasm to change lives.</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>33Z2</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- courseHistory -->
        <div class="large reveal" id="courseHistory" data-reveal>
          <h2>History</h2>
          <p class="lead">Why History?</p>
          <p>History immerses us in exploring evidence from the past and how the consequences of our actions have shaped nations round the World. Analysing different types of sources, using critical and creative thinking and focusing on different periods of history give the subject an exciting prospect for students to study. You will undoubtedly match this passion in your knowledge and enthusiasm which you will want to share with students in inspiring them to enjoy History and perhaps wish to study it further.</p>
          <p>Learn how to teach History with a team of passionate, experienced, excellent teachers. During this one year course you will be taught absolutely everything you need to know to be an effective class practitioner. It all takes place in real classrooms with current History teachers leading the sessions. Educating our next generation takes real drive and enthusiasm. If you are up to the challenge apply to join our School Centred Initial Teacher Training Programme.</p>

          <p class="lead">How is Subject Study structured?</p>
          <p>Subject specific sessions are run by Subject Experts for each specialism and take place for half a day each week. In addition there will be a weekly half day session in school for subject specialist development.</p>
          <p>Subject sessions being delivered by a Subject Expert who is a current outstanding practitioner ensures that you will get up to date advice on subject specific teaching and learning, and the most effective ways of making this outstanding within the classroom.</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>33Z3</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- courseMaths -->
        <div class="large reveal" id="courseMaths" data-reveal>
          <h2>Maths</h2>
          <p class="lead">Why Maths?</p>
          <p>Mathematics is a universal language used to describe anything. It is beautiful, creative and enjoyable. It is essential to everyday life, providing a foundation for understanding the world and making decisions. When we are successful, pupils also see mathematics in this way. However, too many feel it's complicated, irrelevant or difficult - are you the right person to change their mind?</p>
          <p>Learn how to teach Maths with a team of passionate, experienced, excellent teachers. During this one year course you will be taught absolutely everything you need to know to be an effective class practitioner. It all takes place in real Mathematics classrooms with real Mathematics teachers leading the sessions. Educating our next generation takes real drive and enthusiasm. If you are up to the challenge apply to join our School Centred Initial Teacher Training Programme.</p>

          <p class="lead">How is Subject Study structured?</p>
          <p>Subject specific sessions are run by Subject Experts for each specialism and take place for half a day each week. In addition there will be a weekly half day session in school for subject specialist development.</p>
          <p>Subject sessions being delivered by a Subject Expert who is a current outstanding practitioner ensures that you will get up to date advice on subject specific teaching and learning, and the most effective ways of making this outstanding within the classroom.</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>33Z4</td>
            </tr>
            <tr>
              <td>School Direct</td>
              <td>3B7Y</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- courseMFL -->
        <div class="large reveal" id="courseMFL" data-reveal>
          <h2>MFL</h2>
          <p class="lead">Why Modern Foreign Languages (MFL)?</p>
          <p>There are over 6,500 languages in the world and learning any language, whether it be Spanish, French, German or Japanese, enables students to gain a deeper insight into their own culture and raise curiosity about other cultures and the world they live in.</p>
          <p>As an MFL teacher, you can inspire students to discover new cultures and learn languages that could prove beneficial for their career prospects and open up opportunities to travel the world.</p>
          <p>The most popular languages in schools are French, German and Spanish and MFL candidates can access lucrative bursaries while MFL teachers are in high demand.</p>
          <p>Are you an accomplished linguist with a passion for languages and keen to convey your enthusiasm to young people?</p>


          <p class="lead">How is Subject Study structured?</p>
          <p>Subject specific sessions are run by Subject Experts for each specialism and take place for half a day each week. In addition, there will be a weekly half day session in school for subject specialist development.</p>
          <p>Subject sessions being delivered by a Subject Expert who is a current outstanding practitioner ensures that you will get up to date advice on subject specific teaching and learning, and the most effective ways of making this outstanding within the classroom.</p>


          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>39Z9</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- coursePhysics -->
        <div class="large reveal" id="coursePhysics" data-reveal>
          <h2>Physics</h2>
          <p class="lead">Why Physics?</p>
          <p>Physics is a subject regularly in the headlines with news of the latest breakthroughs either in CERN or by NASA. Physics is a subject that helps us to understand the workings of the universe, from the solar system to splitting the atom. It links key ideas of energy, forces and particles.</p>
          <p>Can you make working out frequency of waves a fathomable problem? Does the science of earthquakes provide an awe inspiring learning opportunity? Can you inspire the next Brian Cox?</p>
          <p>As a trainee at The Solent SCITT, you will develop the knowledge and skills to inspire the next generation of Physicists. You will work in four of the region's leading Science departments alongside dedicated teachers with a range of backgrounds and experience. In addition, you will receive tailored training focused on the key challenges faced by the teaching of this dynamic subject.</p>

          <p class="lead">How is Subject Study structured?</p>
          <p>Subject specific sessions are run by Subject Experts for each specialism and take place for half a day each week. In addition there will be a weekly half day session in school for subject specialist development.</p>
          <p>Subject sessions being delivered by a Subject Expert who is a current outstanding practitioner ensures that you will get up to date advice on subject specific teaching and learning, and the most effective ways of making this outstanding within the classroom.</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>2CLX</td>
            </tr>
            <tr>
              <td>School Direct</td>
              <td>3B7Z</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- coursePrimaryTeaching -->
        <div class="large reveal" id="coursePrimaryTeaching" data-reveal>
          <h2>Primary Teaching</h2>
          <p class="lead">Primary Teaching</p>
          <p>At The Solent SCITT, we believe every single pupil deserves access to an excellent primary teacher who will inspire them to love learning and lay the foundations for a bright, exhilarating and rewarding future.</p>

          <p class="lead">How is the Primary Training Programme Structured?</p>
          <p>The Primary Training Programme (PTP) has been developed through collaboration with our partnership schools and offers a bespoke, school-based, high-quality, cross-phase and coherent training programme which is adapted to your individual needs.</p>
          <p>Successful completion of your training with us will result in being awarded Qualified Teacher Status (QTS), along with a Postgraduate Certificate in Education.</p>
          <p>Trainees will have access to outstanding teachers who demonstrate current and innovative practice in the classroom. You will gain experience from within The Solent Teaching School Alliance which includes Infant, Junior, Primary and Special Schools. The length, breadth, and content of the training will be flexible, taking account of your individual previous achievements, strengths and areas of development and ongoing interests.</p>
          <p>The majority of the 50+ days of out of school training, has been developed by experienced teachers and headteachers and will be delivered by expert teachers in partnership schools who have a proven track record in their fields of excellence. Successful candidates will be assigned to a partnership school which will serve as the main training school. All trainees will teach across Key Stage 1 and Key Stage 2 and experience teaching in a second contrasting school within the partnership. This second school experience takes place in the Spring Term and is approximately six weeks long.</p>
          <p>Your school based mentor has responsibility for ensuring that you have an appropriate Individual Training Plan (ITP) and that you have every opportunity to meet the Professional Standards for Qualified Teacher Status. For the duration of your training your timetable will be at a rate which is managed and agreed with your mentor, headteacher and Director of Initial Teacher Training.</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>S56 3496</td>
            </tr>
            <tr>
              <td>School Direct Salaried</td>
              <td>1R1 25SY</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <!-- courseReligiousEducation -->
        <div class="large reveal" id="courseReligiousEducation" data-reveal>
          <h2>Religious Education</h2>
          <p class="lead">Why Religious Education?</p>
          <p>How important is faith to you? How do different faiths affect how different people live their lives?</p>
          <p>Schools have a responsibility for pupils development as informed and responsible citizens with a deep social, spiritual, moral and cultural understanding. The study of Religious Education allows the exploration of open questions and the opportunity for pupils to develop their ideas taking other views into account.</p>
          <p>Extending pupils' religious, ethical and philosophical views can be done in innovative ways. Can you sensitively use controversial issues to develop pupils' views? Can you motivate and inspire pupils' to excel at KS3 and KS4 and pursue the study of Religious Education at a higher level?</p>

          <p class="lead">How is Subject Study structured?</p>
          <p>Subject specific sessions are run by Subject Experts for each specialism and take place for half a day each week. In addition there will be a weekly half day session in school for subject specialist development.</p>
          <p>Subject sessions being delivered by a Subject Expert who is a current outstanding practitioner ensures that you will get up to date advice on subject specific teaching and learning, and the most effective ways of making this outstanding within the classroom.</p>

          <p class="lead">Course Codes</p>
          <table>
            <tr>
              <td>SCITT</td>
              <td>33Z5</td>
            </tr>
          </table>
          <br>

          <button class="close-button" data-close aria-label="Close modal" type="button">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>



        <!-- End of Modals -->

      </div>

        <div class="column text-center">
          <p>Bursaries can be found on the <a href="//getintoteaching.education.gov.uk/funding-and-salary/overview" target="_blank">GetIntoTeaching website</a>.</p>
          <p>The full primary and secondary TLPD programme structure can be accessed here <a href="/assets/full_programme.pdf" target="_blank">Full Programme</a>.</p>
        </div>
    </section>




    <!-- FOOTER -->
    <?php include("includes/footer.html") ?>


    <!-- DO NOT REMOVE -->
    <script src="js/vendor/jquery.js"></script>
    <script src="js/vendor/what-input.js"></script>
    <script src="js/vendor/foundation.js"></script>
    <script src="js/app.js"></script>
    <!-- JQuery toggleSwitch -->
    <script src="js/coursesToggleSwitch.js"></script>
  </body>
</html>
